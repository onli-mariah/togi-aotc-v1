...
const normalizeProductHuntEntry = (entry: any) => {
  // New format: categories.IMMEDIATE ADOPTION, categories.EVALUATE, categories.AVOID
  if (entry.categories) {
    const tools = [
  ...
  // Old format: tools array already exists
  return entry;
};
...
export default function ProductHunt() {
  ...
  const filteredData = useMemo(() => {
    const normalized = data.productHunt.map(normalizeProductHuntEntry);
    ...
    })).filter((entry: any) => entry.tools.length > 0);
  }, [searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container py-6">
          <div className="flex items-center justify-between">
            ...
            <nav className="flex gap-6">
              ...
              <Link href="/producthunt" className="text-foreground hover:text-accent transition-colors">
                Product Hunt
              </Link>
      ...
      <main className="container py-8 space-y-6">
        <h2 className="text-2xl font-bold">Product Hunt Recommendations</h2>
        ...
        {filteredData.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-muted-foreground">
              {searchQuery ? "No tools match your search." : "No Product Hunt recommendations yet. Share Product Hunt links to get started."}
            </CardContent>
          </Card>
        ) : (
                  ...
                  <CardHeader className="cursor-pointer hover:bg-accent/5 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="text-left">
                        ...
                        <CardDescription>
                          {entry.tools.length} tools analyzed
                        </CardDescription>
                      </div>
                      <ChevronDown className={`h-5 w-5 transition-transform ${openEntries[idx] ? 'transform rotate-180' : ''}`} />
                    </div>
                  </CardHeader>
                </CollapsibleTrigger>
                <CollapsibleContent>
                                ...
                                <CardContent className="p-4">
                                    <div className="space-y-2">
                                      <div className="flex items-center gap-2">
                                        ...
                                        {tool.pricing && tool.pricing !== "Unknown" && <Badge variant="outline" className="text-xs">{tool.pricing}</Badge>}
                                      </div>
                                      <p className="text-sm text-muted-foreground">{tool.description}</p>
                                      <p className="text-sm">{tool.businessValue}</p>
                                      <div className="flex gap-3 text-xs">
                                        {tool.productHuntUrl && (
                                          <a href={tool.productHuntUrl} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">
                                            Product Hunt
                                          </a>
                                        )}
                                        {tool.websiteUrl && (
